...
from timm.models._registry import register_model, model_entrypoint
import cv2

__all__ = ['ResNet', 'ResNet_DwtSa', 'BasicBlock', 'Bottleneck']  # model_registry will add each entrypoint fn to this
...
default_cfgs = {
    ...
    #  ResNeXt Dwt_models - Weakly Supervised Pretraining on Instagram Hashtags
    ...
    #  Semi-Supervised ResNe*t Dwt_models from https://github.com/facebookresearch/semi-supervised-ImageNet1K-models
    #  Please note the CC-BY-NC 4.0 license on theses weights, non-commercial use only.
    'ssl_resnet18': _cfg(
    ...
    #  Semi-Weakly Supervised ResNe*t Dwt_models from https://github.com/facebookresearch/semi-supervised-ImageNet1K-models
    #  Please note the CC-BY-NC 4.0 license on theses weights, non-commercial use only.
    'swsl_resnet18': _cfg(
    ...
    #  Squeeze-Excitation ResNets, to eventually replace the Dwt_models in senet.py
    'seresnet18': _cfg(
        url='',
        interpolation='bicubic'),
    'seresnet34': _cfg(
    ...
    #  Squeeze-Excitation ResNeXts, to eventually replace the Dwt_models in senet.py
...
class BasicBlock(nn.Module):
    ...
    # def forward(self, x):
    #     """
    #     forward_features0 or forward_features_ori
    #     :param x:
    #     :return:
    #     """
    #     # SA = x[1]
...
class ResNet_DwtSa(nn.Module):
    ...
    ResNet variants (the same modifications can be used in SE/ResNeXt Dwt_models as well):
    ...
    def __init__(
        ...
        """
        super(ResNet_DwtSa, self).__init__()
        ...
        # Stem
        deep_stem = 'deep' in stem_type                                                    # false
        inplanes = stem_width * 2 if deep_stem else 64                                     # 64
        if deep_stem:
            ...
            self.conv1 = nn.Conv2d(in_chans, inplanes, kernel_size=7, stride=2, padding=3, bias=False)  # stem: 7x7的卷积
        self.bn1 = norm_layer(inplanes)
        self.act1 = act_layer(inplace=True)
        self.feature_info = [dict(num_chs=inplanes, reduction=2, module='act1')]                        # [{'num_chs': 64, 'reduction': 2, 'module': 'act1'}]
        ...
        for stage in stage_modules:
            self.add_module(*stage)  # layer1, layer2, layer3, layer4
        self.feature_info.extend(stage_feature_info)

        # Head (Pooling and Classifier)
        self.num_features = 512 * block.expansion       # 512
    ...
    def forward_features_ori(self, x):
        x = self.conv1(x)      # (2, 3, 224, 224) -->(2, 64, 112, 112)
        x = self.bn1(x)        # (2, 64, 112, 112)
        x = self.act1(x)       # (2, 64, 112, 112)
        x = self.maxpool(x)    # (2, 64, 56, 56)

        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq([self.layer1, self.layer2, self.layer3, self.layer4], x, flatten=True)
        else:
            x = self.layer1(x)  # (2, 64, 56, 56)--> (2, 64, 56, 56)
            x = self.layer2(x)  # (2, 64, 56, 56)--> (2, 128, 28, 28)
            x = self.layer3(x)  # (2, 128, 28, 28)--> (2, 256, 14, 14)
            x = self.layer4(x)  # (2, 256, 14, 14)--> (2, 512, 7, 7)
        return x
    ...
    def forward_features1(self, x):
        ...
        sa1 = SA[0]
        x = x[0]
        ori = (x[0][0].detach().cpu().numpy()*255).astype(np.int)

        x = self.conv1(x)         # (2, 3, 224, 224) -->(2, 64, 112, 112)
        x = self.bn1(x)           # (2, 64, 112, 112)
        x = self.act1(x)          # (2, 64, 112, 112)
        # x_data = x[0]
        # x_feat = np.zeros((8*112, 8*112))
        # for i in range(8):
        #     for j in range(8):
        #         x_feat[i*112:(i+1)*112,j*112:(j+1)*112] = x_data[i*8+j].detach().cpu().numpy()

        x = x * sa1

        # x_data_sa = x[0]
        # x_feat_sa = np.zeros((8*112, 8*112))
        # for i in range(8):
        #     for j in range(8):
        #         x_feat_sa[i*112:(i+1)*112,j*112:(j+1)*112] = x_data_sa[i*8+j].detach().cpu().numpy()

        b = sa1[0][0].detach().cpu().numpy()

        a = (sa1[0][0].detach().cpu().numpy()*255)# .astype(np.int)
        a = cv2.resize(a, (224, 224))
        # print(self.dwt_sa.conv_sa1._modules['body']._modules['0'].weight.data)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x = self.layer1([x, SA])  # (2, 64, 56, 56)--> (2, 64, 56, 56)
            x = self.layer2(x)  # (2, 64, 56, 56)--> (2, 128, 28, 28)
            x = self.layer3(x)  # (2, 128, 28, 28)--> (2, 256, 14, 14)
            x = self.layer4(x)  # (2, 256, 14, 14)--> (2, 512, 7, 7)
            x = x[0]
    ...
    def forward(self, x):
        ...
        a = (sa2[0][0].detach().cpu().numpy()*255)# .astype(np.int)
        a = cv2.resize(a, (224, 224))

        x = [x, SA]
        x = self.forward_features1(x)   # (2, 3, 224, 224) --> (2, 512, 7, 7)

        # x = self.forward_features0(x)   # (2, 3, 224, 224) --> (2, 512, 7, 7)
        x = self.forward_head(x)       # (2, 512, 7, 7) --> (2, num_classes)
        return x
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.forward_features(x)   # (2, 3, 224, 224) --> (2, 512, 7, 7)
        x = self.forward_head(x)       # (2, 512, 7, 7) --> (2, num_classes)
        return x
