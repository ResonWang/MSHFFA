...
from task.Dwt_models import common
...
class VanillaNet(nn.Module):
    ...
    def forward(self, x):
        if self.deploy:
            ...
            x = self.stem1(x)         # 224-->56
            x = torch.nn.functional.leaky_relu(x,self.act_learn)
            x = self.stem2(x)         # 56-->56
...
class VanillaNet_DwtSa(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, dims=[96, 192, 384, 768],
                 drop_rate=0, act_num=3, strides=[2, 2, 2, 1], deploy=False, ada_pool=None, **kwargs):
        super().__init__()
        self.deploy = deploy
    ...
    def forward(self, x):
        ...
        if self.deploy:
            ...
            x = self.stem1(x)
            # x = x * sa2      # a = (sa2[0][0].detach().cpu().numpy()*255).astype(np.int)
            x = torch.nn.functional.leaky_relu(x, self.act_learn)
            # x = x * sa2      # a = (sa2[0][0].detach().cpu().numpy()*255).astype(np.int)
            x = self.stem2(x)    # 56-->56
            x = x * sa2      # a = (sa2[0][0].detach().cpu().numpy()*255).astype(np.int)
        ...
        for i in range(self.depth):
            x = self.stages[i](x)  # 8: 56-->56, 56-->28, 28-->14, 14-->14, 14-->7, 7-->7  5: 56->28, 28->14, 14->7
            # if i == 0:
            #     x = x * sa3
...
class VanillaNet_DwtSa112(nn.Module):  # 自己额外添加了一个112的层
    def __init__(self, in_chans=3, num_classes=1000, dims=[96, 192, 384, 768],
                 drop_rate=0, act_num=3, strides=[2, 2, 2, 1], deploy=False, ada_pool=None, **kwargs):
        super().__init__()
        self.deploy = deploy
...
# model = vanillanet_8(True,num_classes=3)
# n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
# print('number of params:', n_parameters)
# input = torch.randn((2,3,224,224))
# y = model(input)                     # 输出logits
# print(y.shape)